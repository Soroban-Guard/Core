...
fn test_unchecked_arithmetic_flagged_high() {
    ...
    assert!(has_rule(&findings, "O-01"), "expected O-01, got: {findings:?}");
...
fn test_division_by_variable_flagged() {
    let findings = findings_for_source(
        "pub fn div(a: i128, b: i128) -> i128 { a / b }",
    );
    assert!(has_rule(&findings, "O-03"), "expected O-03, got: {findings:?}");
...
fn test_loop_accumulation_flagged_high() {
    ...
    assert!(has_rule(&findings, "O-04"), "expected O-04, got: {findings:?}");
...
fn test_cast_truncation_flagged_low() {
    let findings = findings_for_source(
        "pub fn shrink(amount: i128) -> u32 { amount as u32 }",
    );
    assert!(has_rule(&findings, "O-05"), "expected O-05, got: {findings:?}");
...
fn test_arithmetic_compared_is_o02_not_o01() {
    let findings = findings_for_source(
        "pub fn check(a: i128, b: i128, max: i128) -> bool { a + b > max }",
    );
    assert!(has_rule(&findings, "O-02"), "expected O-02, got: {findings:?}");
...
fn test_unit_counter_demoted_to_medium() {
    let findings = findings_for_source(
        "pub fn count(mut i: i128) -> i128 { i += 1; i }",
    );
    let o01 = rule(&findings, "O-01");
